...
export interface IRecognizeOptions {
  maxJointDistanceThreshold: number; // 0.05 is fine 
  compareOnlyFingerTips?: boolean; // possible optimization
}
...
export class HandGestureRecognition {
  ...
  recognizeGesture(joints: any): IRecognitionResult {
    ...
    for (const gesture of this.gestureDatabase) {
      ...
      for (const key in gesture.data) {
        ...
        if(this.options.compareOnlyFingerTips && !key.includes("-tip")) continue;
  ...
  public static CreateDefault(type: ControllerType) {
    ...
    return new HandGestureRecognition(
      type == ControllerType.Left ? leftDatabase : rightDatabase,
      { maxJointDistanceThreshold: 0.06, compareOnlyFingerTips : false }
    );
  }
}
